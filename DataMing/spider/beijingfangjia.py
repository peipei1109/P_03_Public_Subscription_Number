...
#7月1号  整站40万条房价数据并行抓取，可更换抓取城市
#http://mp.weixin.qq.com/s?__biz=MzA3MDg0MjgxNQ==&mid=2652389861&idx=1&sn=3dc97ec82c6eaf23528e435145b921dc&scene=4#wechat_redirect


#目前有bug，因为页面有变动，url有变动，而且也找不到最大页面数~~
...
def get_page(url):
    headers = {
        'User-Agent': r'Mozilla/5.0 (Windows NT 10.0; WOW64; rv:46.0) Gecko/20100101 Firefox/46.0',
...
def get_info_list(search_dict,layer, tmp_list, search_list):
    layer+=1 #设置字典层级
    search_key=list(search_dict.keys())
    for i in range(len(search_key)):
        tmp_key=search_key[i]  #提取当前字段层级key
        tmp_list.append(tmp_key) #将当前key值作为索引提娜佳到tmp_list
